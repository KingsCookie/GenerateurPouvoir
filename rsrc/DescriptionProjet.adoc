...
Un pouvoir de base est composé du nombre minimal de traits indiqué ci-dessus. Si un individu hérite de traits actifs supplémentaires compatibles avec ce pouvoir, ceux-ci *enrichissent* le pouvoir existant. Exemple : un individu avec _Partie du corps : Bras_, _Etat : Lumineux_ et _Etat : Vibrant_ en actif aura un pouvoir "Bras Lumineux et Vibrant".
...
. *tire d'abord* si l'individu *possède un pouvoir ou non*, selon le pourcentage donné par l'utilisateur ;
. *uniquement si oui* : tire un *type de pouvoir* parmi les 5, pondéré ;
. tire les *traits nécessaires* à former ce pouvoir (selon les pondérations des traits) ;
. attribue à chaque trait une *résilience initiale* paramétrable.
...
+
. *Fichier de configuration seule* : tous les paramètres, catalogues de traits, catalogue d'espèces et leurs genres, pondérations, facteurs bonus/malus, seuils, etc.
. *Fichier de données générées seul* : tous les individus, la généalogie, l'historique.
. *Fichier combiné* : configuration + données dans un seul fichier.
+
...
. *Algorithme de construction des pouvoirs à partir des traits actifs* — règles exactes pour combiner les traits actifs d'un individu en un (ou plusieurs) pouvoirs lisibles, notamment quand plusieurs traits du même type sont actifs (cf. §3.2).
. *Stratégie d'appariement* en simulation automatique (totalement aléatoire ? contraintes par espèce, génération, lien de parenté ?).
. *Statistiques affichées* en mode simulation automatique.
. *Format précis* du fichier d'export / import.
. *Maquettes UI* des différents écrans.
