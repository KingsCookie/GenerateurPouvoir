...
NOTE: La notion de *génération* comme champ de la personne *disparaît* (cf. <<no-gen>>). Elle ne subsiste que comme *regroupement d'affichage* (tranches de 20 ans), utilisé en filtre (cf. <<ui-liste>>).
...
Subsiste uniquement une *génération d'affichage* = tranche de *20 ans* de l'année de naissance (utilisée comme filtre, cf. <<ui-liste>>).
...
* Constante *`K`* (duplication de traits et génération de traits `K…` dans l'algorithme <<algo-pouvoirs>>).
