...
L'ensemble du comportement aléatoire est entièrement *déterministe* : il découle d'une seule *seed* (cf. <<seed>>).
...
* La seed fait partie des *données de configuration exportées* (cf. <<persistance>>) et est *restaurée* lors du rechargement d'une configuration.

NOTE: La reproductibilité repose sur _même seed + même séquence d'actions utilisateur_. Les données exportées contiennent par ailleurs les résultats déjà produits.
...
Un *pouvoir* est une combinaison de traits, assortie d'une *puissance* et d'une *maîtrise* (cf. <<puissance>>).

Contrairement à la V1, *les « types de pouvoirs » n'existent plus* comme structure permanente. Ils ne subsistent *que pour la génération d'un pouvoir en cas de mutation forte* (cf. <<cas-speciaux>>). Dans tous les autres cas, les pouvoirs d'un individu sont *dérivés de son ADN* par l'algorithme de transformation traits -> pouvoirs (cf. <<algo-pouvoirs>>).

Les pouvoirs (et leur puissance/maîtrise) sont *calculés une seule fois, à la naissance* de l'individu, puis *stockés* dans sa description (cf. <<personnes>>). Ils ne sont pas recalculés ensuite.
...
| `sexe / genre` | *Enum paramétrable par espèce* (cf. <<especes>>). Le genre `"tout"` est présent par défaut chez toutes les espèces.
| `espèce` | *Enum* appartenant au catalogue des espèces (cf. <<especes>>). Défaut : `"humain"`.
| `date de naissance` | Date précise (jour, mois, année). Le temps « courant » est toujours le 1ᵉʳ janvier d'une année (cf. <<temps>>).
| `âge` | Entier = `année courante − année de naissance`. Tout le monde naît à *0 an*.
| `vivant / décédé` | Booléen. Si décédé : champ *obligatoire `raison du décès`* (texte libre). Voir <<mort>>.
...
| `conjoints` | Liste de conjoints, chacun marqué `actuel` ou `ex` (cf. <<tick>>).
| `ADN` | Objet *ADN* : liste de *triplets* `(trait, actif/inactif, résilience)` (cf. <<adn>>). Peut être vide.
| `pouvoir(s)` | Liste de pouvoirs, chacun avec sa *puissance* et sa *maîtrise* (cf. <<puissance>>). Calculée à la naissance. Peut être vide.
...
NOTE: La notion de *génération* comme champ de la personne *disparaît* (cf. <<no-gen>>). Elle ne subsiste que comme *regroupement d'affichage* (tranches de 20 ans), *affiché sur la fiche de l'individu* (cf. <<ui-fiche>>) et utilisé pour les *tris et filtres* (cf. <<ui-liste>>).
...
* chaque espèce porte ses propres *paramètres de reproduction et de portée* (cf. <<params-espece>>).
...
* seules les personnes *seules* (célibataires ou divorcées) peuvent former un *nouveau* couple (cf. <<tick>>).
...
* C'est l'ADN qui porte les traits hérités d'un individu (champ `ADN` du <<personnes>>).
* La *résilience* est le pourcentage de chance que le trait soit transmis *actif* à la descendance.
* Un trait peut être présent en ADN tout en étant *inactif* : il est quand même transmissible (cf. <<transmission-inactifs>>).

[[heredite]]
== 4. Hérédité — Modèle de résilience

NOTE: Ce modèle est *inchangé par rapport à la V1* ; seule la terminologie « mutation » devient « *mutation forte* » (cf. <<cas-speciaux>>), et la représentation des traits d'un individu est désormais l'objet *ADN* (<<adn>>).
...
. *Tirage du cas spécial* (cf. <<cas-speciaux>>), avec leurs taux respectifs :
** *mutation forte*, ou
** *naissance sans pouvoir*, ou
** *naissance normale* (cas par défaut).
. *Constitution de l'ADN* :
** _Naissance normale_ : hérédité standard (<<heredite>>) à partir de tous les parents.
** _Mutation forte_ / _sans pouvoir_ : tous les traits parentaux sont hérités *inactifs* (cf. <<cas-speciaux>>).
. *Mutation faible* (cf. <<mutation-faible>>) — *uniquement pour les naissances normales* : tirages indépendants de gain et de perte de trait, appliqués à l'ADN.
. *Calcul des pouvoirs* :
** _Naissance normale_ : algorithme traits -> pouvoirs (<<algo-pouvoirs>>) sur les traits *actifs* de l'ADN.
** _Mutation forte_ : un unique pouvoir issu du gabarit tiré (<<cas-speciaux>>).
** _Sans pouvoir_ : aucun pouvoir.
. *Calcul de la puissance et de la maîtrise* de chaque pouvoir (<<puissance>>).
...
Chaque trait est tiré aléatoirement dans la liste des traits de son type, *avec les pondérations* (<<params-generation>>).
====

NOTE: Le type *AEP* (Action + Élément + Partie du corps) de la V1 *disparaît* du gabarit. Il pourra toutefois *émerger* de façon dérivée via l'algorithme traits -> pouvoirs (<<algo-pouvoirs>>).
...
La notion de *génération* (champ de personne, numérotation, `max(parents)+1`) *disparaît*. Le temps est désormais géré par des *dates* et l'*avancement par années* (<<temps>>).

Subsiste uniquement une *génération d'affichage* = tranche de *20 ans* de l'année de naissance (par ex. génération 0 = années [0, 19], génération 1 = [20, 39], etc.). Elle est *calculée à partir de l'année de naissance*, *affichée sur la fiche de l'individu* (cf. <<ui-fiche>>) et sert aux *tris et filtres* (cf. <<ui-liste>>).
...
* la *constante de duplication `D`* (cf. <<sous-listes>>), qui pilote la probabilité de duplication des traits secondaires ;
* la *constante de génération `K`* (cf. <<sous-listes-pouvoir>>), qui pilote la probabilité de génération de nouveaux traits (`Ka`, `Ke`, …).

TIP: Si la personne n'a *aucun trait actif*, elle est *sans pouvoir*.
...
*Duplication* : au moment où l'on assigne un trait secondaire, il y a `(résilience du trait secondaire / D) %` de chance que ce trait *se duplique*, c'est-à-dire qu'une *copie* soit aussi placée dans *une autre* sous-liste. (`D` = constante de duplication, distincte de la constante de génération `K` du <<sous-listes-pouvoir>>.)
...
* `Ka` / `Ke` / `Kp` / `Kaj` / `Kr` / `Ket` : *`K` % de chance* de générer *UN SEUL* nouveau trait du type indiqué (`K` = constante de génération, distincte de la constante de duplication `D` du <<sous-listes>>).
...
* Le bouton « *reproduire tout* » de la V1 *disparaît*, remplacé par un bouton « *avancer de X années* » (X *réglable*, minimum *1 an*). Avancer fait *défiler la date* (année par année) et applique, *pour chaque année*, le *tick annuel* (<<tick>>).
* *Plus personne ne meurt naturellement* : il n'y a plus d'âge de mort, et l'*option « immortel » disparaît*. Seul l'utilisateur peut tuer un individu (cf. <<mort>>).
...
* Chaque personne *célibataire ou divorcée*, et *pas plus vieille que l'âge maximal de reproduction* de son espèce, tire individuellement, selon la *gaussienne de reproduction* (<<params-espece>>), le *% de chance qu'elle veuille se reproduire* cette année.
* On constitue la *liste des candidats* et on tire *aléatoirement des groupes* de la *taille requise par l'espèce*, en respectant les *règles de consanguinité* (<<consanguinite>>) et *sans inter-espèces*.
...
* Les *couples déjà formés* ont chaque année un *% de chance de se reproduire* *issu de la même gaussienne de reproduction* (<<params-espece>>) que celle des candidats célibataires (évaluée selon l'âge des membres du couple) ; ce pourcentage reste *éditable couple par couple* par l'utilisateur.
* Toute reproduction produit une *portée* (<<portee>>).

NOTE: Le genre `"tout"` peut être groupé avec n'importe quel genre, mais *jamais en inter-espèces* et *sans briser* de couple existant (<<especes>>).
...
Le nombre d'enfants d'une reproduction dépend de l'espèce, via trois paramètres (cf. <<params-espece>>) :
...
Chaque pouvoir possède une *puissance* et une *maîtrise* (valeurs entières). Elles ne sont *bornées à [1, 10] que lorsqu'une nouvelle valeur aléatoire est tirée* (mutation forte, ou cas _A_ d'une naissance — cf. <<puissance-naissance>>). Dans tous les autres cas (valeurs dérivées de la moyenne des parents), *il n'y a aucune borne*, ni inférieure ni supérieure : par exemple, des parents de puissance moyenne 10 peuvent donner un enfant de puissance 11.
...
Les pouvoirs de l'enfant ont été calculés (<<algo-pouvoirs>>). Pour leur attribuer puissance/maîtrise :
...
Un clic sur un individu ouvre sa *fiche individu* (<<ui-fiche>>).
...
* ses *informations globales* (nom, date de naissance, âge, *génération* [tranche de 20 ans, cf. <<no-gen>>], espèce, genre, statut vivant/décédé, notes, parents, conjoints…) ;
* son *ADN / ses traits*, affichés selon le mode d'affichage actif (cf. <<ui-modes>>) ;
...
Page qui affiche *le même arbre* que la fiche (<<ui-fiche>>), centré sur le même individu, même profondeur N, mais *sans informations latérales* : seulement l'arbre, en grand. Un clic sur une case recentre l'arbre.

=== 8.4. Autres pages

* *Écran d'avancement du temps* : bouton « *avancer de X années* » (X réglable, min. 1).
* *Écran sandbox* : voir <<sandbox>>. La *reproduction manuelle* (sélection de 1, 2 ou plusieurs individus) n'est disponible *qu'en sandbox*.
* *Page des paramètres* : voir <<parametrage>> ; affiche notamment la *seed* (+ bouton de régénération) et les *courbes gaussiennes* de reproduction par espèce.
...
* Constante de *duplication `D`* : pilote la duplication des traits secondaires (<<sous-listes>>).
* Constante de *génération `K`* : pilote la génération de nouveaux traits `K…` (<<sous-listes-pouvoir>>). *Distincte de `D`.*
...
* Probabilités *B* et *C* de puissance/maîtrise (<<puissance>>) ; *A = 100 − 2·B − C* affichée.
...
NOTE: Plus de durée de vie, plus d'âge de mort, plus d'âge à la naissance (toujours 0), plus d'immortalité : ces paramètres V1 *disparaissent*.
...
* Les *6 types de traits* sont *fixés* dans la V2. Les « types de pouvoirs » ne subsistent que pour la mutation forte (<<cas-speciaux>>).

== 10. Modes d'utilisation

=== 10.1. Avancement du temps (mode principal)

L'utilisateur fait *avancer le temps de X années*. Chaque année déclenche le *tick annuel* (<<tick>>) : divorces, mises en couple, reproductions et naissances, et le vieillissement de toute la population.

=== 10.2. Reproduction manuelle (sandbox uniquement)

La sélection manuelle de *1, 2 ou plusieurs* individus pour déclencher une reproduction n'est disponible *qu'en sandbox* (<<sandbox>>).
