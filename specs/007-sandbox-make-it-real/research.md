...
## D11 — Édition directe du cycle de vie conjugal (BUG-001 volet B)

- **Décision** : ajouter trois fonctions **pures** au cœur sandbox — `formCouple(state, aId, bId, year)`
  (couple « actuel », conjoints symétriques, émet `couple{year}`), `divorceCouple(state, coupleId, year)`
  (conjoints → « ex », émet `divorce{year}`, couple inactif), `dissolveConjugalLink(state, coupleId)`
  (retrait symétrique du lien + **purge** des événements `couple`/`divorce` du couple → retour célibataire).
  Elles **ne touchent jamais** `parents`/`enfants` (INV-S12) ; la parenté reste réservée à la reproduction
  (FR-011b). Le **volet A** (formulaire exposant ADN/pouvoirs/`raisonDeces`) est une **correction UI** : le
  cœur (`PersonDraft`/`editPerson`) accepte déjà ces champs (dérive d'implémentation, pas de changement cœur).
- **Rationale** : la demande (« la création/édition permet de TOUT FAIRE ») exige le cycle de vie complet ;
  réutiliser le **journal d'événements daté** (D7) garantit que les couples édités à la main se
  **reconstruisent** correctement à l'année (INV-S9). Opérations dédiées (≠ `editPerson`) pour préserver
  la **symétrie** des liens et l'invariant INV-S5.
- **Alternatives rejetées** : intégrer `conjoints` dans `PersonPatch` (casse la symétrie et INV-S5 ; un
  patch unilatéral laisserait l'autre conjoint incohérent) ; ne pas émettre d'événements (la reconstruction
  historique deviendrait fausse pour les couples édités manuellement). **Aucune dépendance ajoutée.**

## D12 — Parité de filtrage dans la sandbox (BUG-002)

- **Décision** : la `SandboxView` réutilise le composant **`FilterBar`** et le moteur **pur**
  `filterPopulation` (Feature 4/5), appliqués à la population **reconstruite** à l'année (`reconstructAtYear`).
  Le store `criteria` peut être partagé ou dupliqué pour la sandbox (choix d'implémentation UI). En **mode
  reproduction manuelle**, la sélection de parents (un `Set` d'ids) est **indépendante** des lignes
  affichées : un parent masqué par un filtre **reste** sélectionné.
- **Rationale** : moteur déjà **pur et disponible** ⇒ correction **UI seule**, **aucune dépendance** ;
  cohérence d'expérience entre liste principale et sandbox.
- **Alternatives rejetées** : réécrire un filtrage spécifique sandbox (duplication inutile) ; filtrer en
  amont de la reconstruction (l'ordre correct est **reconstruire à l'année → filtrer**).
