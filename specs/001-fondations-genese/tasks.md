...
- [x] T035 [P] [US1] **Formateur de libellé par gabarit** `formatPowerLabel(template, labelA, labelB)` dans `src/core/genesis/derived.ts` → AE `{action} {élément}`, PE `{partie} en {état}`, PA `{ajout} sur {partie}`, PR `{remplacement} à la place de {partie}` (FR-024). Réutilisé par `powerLabel` (T017) **et** `generateStrongMutationPower` (T018). *(BUG-001)*
- [x] T036 [P] [US1] Tests du formateur dans `tests/unit/power-label.test.ts` : vérifie les **4 formats** AE/PE/PA/PR (ordre des traits + connecteurs « en / sur / à la place de ») et la cohérence `powerLabel` ↔ `label` stocké. *(BUG-001)*
